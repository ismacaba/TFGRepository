...
/**
 * Servlet implementation class admin_derechos
 */
@WebServlet("/admin_derechos")
public class admin_derechos extends HttpServlet {
	...
	public void devolverPaginaHTML(HttpServletResponse resp) throws ServletException {
		...
		if (taccion.equals("V")) { 
			out.println("<br/><br/>&nbsp;&nbsp;<a href='admin_derechos_alta?taccion=form&tvalidated=s'><strong>ALTA DE NUEVO DERECHO</strong></a><br/><br/>");
			out.println("<br/>Listado de derechos RGPD guardados en la aplicación, seleccione el que desea modificar.<br/><br/>");			
			...
			try {
				...
				ResultSet rs = stmt.executeQuery("select * from rgdp_derechos order by idderecho");
				
				out.println("<table style='float:left; border: 1px solid black; width:750px; border-collapse:collapse;'><tr>");
				out.println("<td width='150px' style='border: 1px solid black; padding:5px; margin:0px;'><label><strong>ID_DERECHO</strong></label></td>");
				out.println("<td width='400px' style='border: 1px solid black; padding:5px; margin:0px;'><label><strong>DERECHO RGPD</strong></label></td>");
				...
				while (rs.next()) {                
					...
					out.println("<tr><td width='150px' style='border: 1px solid black; padding:5px;'><label>" + campo1 + "</label></td><td width='400px' style='border: 1px solid black; padding:5px;'><label><strong><a href='admin_derechos_mod?idderecho=" + campo1 +  "&tvalidated=s&taccion=form'>"  + campo2 + "</a></strong></label></td><td width='100px' style='border: 1px solid black; padding:5px;'><label>" + campo3 + "</label></td></tr>");
				}
				rs.close();

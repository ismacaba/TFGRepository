...
/**
 * Servlet implementation class admin_empresas
 */
@WebServlet("/admin_empresas")
public class admin_empresas extends HttpServlet {
	...
	public void devolverPaginaHTML(HttpServletResponse resp) throws ServletException {
		...
		if (taccion.equals("V")) { 
			out.println("<br/><br/>&nbsp;&nbsp;<a href='admin_empresas_alta?taccion=form&tvalidated=s'><strong>ALTA DE NUEVA EMPRESA</strong></a><br/><br/>");
			out.println("<br/>Listado de empresas guardadas en la aplicación, seleccione la que desea modificar.<br/><br/>");			
			...
			try {
				...
				//Recuperamos registros de la tabla Empresas solo las que están ACTIVAS				
				ResultSet rs = stmt.executeQuery("select idempresa, cif, razonsocial, telefono1, emaildpto, estado_empresa from rgdp_empresas order by razonsocial");
				...
				out.println("<td width='200px' style='border: 1px solid black; padding:5px; margin:0px;'><label><strong>EMAIL RGPD</strong></label></td>");
				...
				while (rs.next()) {                
					...
					campo3 = rs.getString(3); // Empresa/Razón Social
					campo4 = rs.getString(4); // Teléfono
					...
					out.println("<td width='200px' style='border: 1px solid black; padding:5px;'><label><strong><a href='admin_empresas_mod?idempresa=" + campo1 +  "&tvalidated=s&taccion=form'>"  + campo3 + "</a></strong></label></td>");
					out.println("<td width='100px' style='border: 1px solid black; padding:5px;'><label>" + campo4 + "</label></td>");
					out.println("<td width='200px' style='border: 1px solid black; padding:5px;'><label>" + campo5 + "</label></td>");
					out.println("<td width='100px' style='border: 1px solid black; padding:5px;'><label>" + campo6 + "</label></td></tr>");
				}
				rs.close();
